...
<form action="{{ route('siswa.update', $siswa->id) }}" method="POST" enctype="multipart/form-data">
  ...
  <input type=" text" name="nis" value="{{ old('nis', $siswa->) }}" required>
  <br><br>

  <label>Nama Lengkap</label><br>
  <input type="text" id="name" value="{{ pld('name, $siswa->) }}" required>
  <br><br>

  <label>Tingkatan</label><br>
  <select name="tingkatan"required> 
    <option {{ 'X' == $siswa->tingkatan ? 'selected' : '' }} value="X"<option>
    <option {{ 'XI' == $siswa->tingkatan ? 'selected' : '' }} value="XI"<option>
    <option {{ 'XII' == $siswa->tingkatan ? 'selected' : '' }} value="XII"<option>
  </select>
  <br><br>
  ...
  <select name="status" required>
    ...
    <option {{ '0' == $siswa->status ? 'selected' : '' }} value="0">Aktif</option>
  </select>
  <br><br>
